...
const paletteTransformer = (node, doc, map) => {
  ...
  const keys = ['travelr', 'palette', ...key.split('.')];

  // @ts-ignore
  const value = doc.getIn(keys);
  if (value === undefined) {
    throw map.errorForRange(
      `Could not resolve '${key}' in travelr.palette`,
      node.range,
    );
  }
...
const colorTransformer = (node, doc, map) => {
  ...
  const keys = ['travelr', 'colors', ...key.split('.')];

  // Start by checking if the value is a reference to travelr.colors.
  // @ts-ignore
...
const fontFamilyTransformer = (node, doc, map) => {
  ...
  const key = String(nodeValue);
  const keys = ['travelr', 'typography', 'font-family', ...key.split('.')];

  // @ts-ignore
  const value = doc.getIn(keys, /* keep node */ true);
  if (value === undefined) {
    throw map.errorForRange(
      `Could not resolve '${key}' in travelr.colors`,
      node.range,
    );
  }
...
function getScalarVisitor(path) {
  // You are here: ['travelr', 'colors', ...]
  if (path.length > 2 && path[0] === 'travelr' && path[1] === 'colors') {
    return paletteTransformer;
  }

  // NB. You are here: ['travelr', 'typography', 'display', *, key]
  if (
    path.length === 5 &&
    path[0] === 'travelr' &&
    path[1] === 'typography' &&
    ...
    return (node, doc, map) => {
      ...
      const key = nodeValue;
      const keys = ['travelr', 'typography', prefix];

      // First, attempt a lookup of the typography at travelr.typography - this'll weed
...
function transform(parsed) {
   ...
   * @param {readonly (string | number)[]} path
   *
   * @return {import('./types').TravelrData}
   */
  function createDataObject(node, path) {
    ...
    if (node.type === 'MAP') {
      /** @type {import('./types').TravelrObject} */
    ...
    if (node.type === 'SEQ') {
      /** @type {import('./types').TravelrArray} */
      const data = [];
